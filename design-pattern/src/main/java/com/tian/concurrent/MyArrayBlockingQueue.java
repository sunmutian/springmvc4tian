...
public class MyArrayBlockingQueue<E> extends AbstractQueue<E>
    ...
    void removeAt(final int removeIndex) {
        ...
        if (removeIndex == takeIndex) {
            ...
            if (++takeIndex == items.length)
                takeIndex = 0;
            count--;//队列元素减1
            if (itrs != null)   itrs.elementDequeued();//更新迭代器中的数据
        } else {
    ...
    /**
     * Creates an {@code MyArrayBlockingQueue} with the given (fixed)
    ...
    public E take() throws InterruptedException {
        ...
        try {
            while (count == 0)//如果队列没有元素
                notEmpty.await();//执行阻塞操作
    ...
    class Itrs {
         ...
         */
        private class Node extends WeakReference<java.util.concurrent.ArrayBlockingQueue.Itr> {
            java.util.concurrent.ArrayBlockingQueue.Itrs.Node next;

            Node(java.util.concurrent.ArrayBlockingQueue.Itr iterator, java.util.concurrent.ArrayBlockingQueue.Itrs.Node next) {
                super(iterator);
                this.next = next;
            }
        }
        ...
        Itrs(java.util.concurrent.ArrayBlockingQueue.Itr initial) {
            register(initial);
        }
        ...
        void doSomeSweeping(boolean tryHarder) {
            ...
            java.util.concurrent.ArrayBlockingQueue.Itrs.Node o, p;
            final java.util.concurrent.ArrayBlockingQueue.Itrs.Node sweeper = this.sweeper;
            ...
            for (; probes > 0; probes--) {
                ...
                final java.util.concurrent.ArrayBlockingQueue.Itr it = p.get();
                final java.util.concurrent.ArrayBlockingQueue.Itrs.Node next = p.next;
         ...
         */
        void register(java.util.concurrent.ArrayBlockingQueue.Itr itr) {
            // assert lock.getHoldCount() == 1;
            head = new java.util.concurrent.ArrayBlockingQueue.Itrs.Node(itr, head);
        }
        ...
        void takeIndexWrapped() {
            // assert lock.getHoldCount() == 1;
            cycles++;
            for (java.util.concurrent.ArrayBlockingQueue.Itrs.Node o = null, p = head; p != null;) {
                final java.util.concurrent.ArrayBlockingQueue.Itr it = p.get();
                final java.util.concurrent.ArrayBlockingQueue.Itrs.Node next = p.next;
        ...
        void queueIsEmpty() {
            // assert lock.getHoldCount() == 1;
            for (java.util.concurrent.ArrayBlockingQueue.Itrs.Node p = head; p != null; p = p.next) {
                java.util.concurrent.ArrayBlockingQueue.Itr it = p.get();
                if (it != null) {
                    p.clear();
                    it.shutdown();
                }
    ...
    /**
     * Iterator for MyArrayBlockingQueue.
     *
        ...
        Itr() {
            ...
            try {
                if (count == 0) {
                    ...
                    final int takeIndex = java.util.concurrent.ArrayBlockingQueue.this.takeIndex;
                    prevTakeIndex = takeIndex;
                    nextItem = itemAt(nextIndex = takeIndex);
                    cursor = incCursor(takeIndex);
                    if (itrs == null) {
                        itrs = new java.util.concurrent.ArrayBlockingQueue.Itrs(this);
                    } else {
